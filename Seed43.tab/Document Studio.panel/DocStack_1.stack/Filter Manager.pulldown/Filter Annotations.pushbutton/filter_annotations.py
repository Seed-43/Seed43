...
PARAM_CANDIDATES = [
    ...
    (int(DB.BuiltInParameter.ALL_MODEL_TYPE_NAME), False),
    (int(DB.BuiltInParameter.DATUM_TEXT),          False),
    (int(DB.BuiltInParameter.ROOM_NAME),           False),
    (int(DB.BuiltInParameter.ALL_MODEL_MARK),      False),
]
    ...
    for (pid_int, use_vs) in PARAM_CANDIDATES:
        ...
        for host in [element_type, element]:
            ...
            try:
                ...
                if p is None:
                    try:
                        bip = DB.BuiltInParameter(pid_int)
        ...
        try:
            ...
            ef        = DB.ElementParameterFilter(rule)
            ...
            with revit.Transaction("Test filter param"):
                pf = DB.ParameterFilterElement.Create(doc, test_name, cat_ids, ef)
                doc.Delete(pf.Id)

            return param_id, value

        except Exception as ex:
            logger.debug("Param {} failed for {}: {}".format(
                pid_int, category.Name, str(ex)))
            continue
...
def create_name_filter(filter_name, category, param_id, value):
    ...
    rule      = DB.FilterStringRule(provider, evaluator, value)
    ef        = DB.ElementParameterFilter(rule)
    return DB.ParameterFilterElement.Create(doc, filter_name, cat_ids, ef)
            ...
            while True:
                try:
                    ...
                    if param_id is not None:
                        ...
                        filter_name       = FILTER_NAME_FORMAT_ALL.format(
                            category.Name)
