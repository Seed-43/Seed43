...
BUILTIN_PARAM      = DB.BuiltInParameter.SYMBOL_NAME_PARAM
...
def main():
    try:
        with forms.WarningBar(
                title="Filter Element (Host): Select element. ESC to cancel."):
            while True:
                try:
                    ...
                    with revit.Transaction("Create Parameter Filter"):
                        param_filter = create_parameter_filter(
                            ...
                            DB.ElementId(BUILTIN_PARAM), element_name, doc)
                        if param_filter:
