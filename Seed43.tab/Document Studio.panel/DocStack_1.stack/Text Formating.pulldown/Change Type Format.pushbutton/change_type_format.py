...
source_type_ids = {
    ...
    if tn.get_Parameter(
        DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString() == source_type_name
}
...
selected_formats = forms.SelectFromList.show(
    format_options,
    title="Select Formatting (optional):",
    multiselect=True)
...
all_textnotes = DB.FilteredElementCollector(doc).OfClass(
    DB.TextNote).ToElements()
source_texts  = [tn for tn in all_textnotes
                 if tn.TextNoteType.Id in source_type_ids]
...
if doc.IsWorkshared:
    ...
    if worksets_to_edit:
        try:
            ...
            output.print_md("# Failed to make worksets editable: {}".format(
                str(e)))
            script.exit()
...
try:
    if doc.IsWorkshared:
        with revit.TransactionGroup("Change Type and Format"):
            with revit.Transaction("Change Type and Format"):
                for text in source_texts:
                    if target_type_id and \
                            text.TextNoteType.Id != target_type_id:
                        try:
                            text.ChangeTypeId(target_type_id)
                        except Exception:
                            pass
                        try:
                            type_param = text.get_Parameter(
                                DB.BuiltInParameter.SYMBOL_ID_PARAM)
