...
tools/pylink_excel.py. Word-specific logic (docx parsing, the
Strict-layout algorithm, section groups, the Word side of the
card/row UI) lives in tools/pylink_word.py. Both are mixed into
PyLinkWindow below via multiple inheritance, so every method still
...
class PyLinkWindow(forms.WPFWindow, ExcelCardMixin, WordCardMixin):
    ...
    def _save_persisted_state(self):
        """Write current UI state to the pyLink shared parameter."""
        try:
            save_pylink_state(self._file_data)
        except Exception as ex:
            logger.warning('pyLink save: {}'.format(ex))

    def _load_persisted_state(self):
        """
        Read pyLink shared parameter and rebuild cards + rows.
        ...
        try:
            ...
            logger.warning('pyLink load: {}'.format(ex))
            return
        if not cards:
            return
        ...
        for card in cards:
            ...
            if not path or not os.path.exists(check_path):
                logger.warning('pyLink restore: file not found: {}'.format(check_path))
                continue
            ...
            for rdata in card.get('rows', []):
                ...
                # been renamed outside pyLink since the last apply, so
    ...
    def _check_row_status(self, row, card_key):
        ...
        try:
            ...
            try:
                ...
                logger.warning('pyLink hash check failed: {}'.format(ex))
                return 'success'
    ...
    def _card_unlink_view(self, path):
        ...
        rows stay exactly as they are, but pyLink stops checking the
    ...
    def _card_remove_views(self, path):
        ...
        from pyrevit.revit import query
        removed = 0
        with revit.Transaction('pyLink - Remove views'):
            for row in rows:
                view_name = row.ViewName.strip()
                for v in query.get_elements_by_class(DB.View, doc=doc):
                    try:
