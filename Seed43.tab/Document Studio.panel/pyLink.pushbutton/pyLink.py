# -*- coding: utf-8 -*-
"""
PyLink - export model/schedule parameters to Excel or ODS, edit externally,
...
from pylink_io import write_workbook, read_workbook, HIDDEN_COLUMNS
...
class MainWindow(forms.WPFWindow):
    ...
    def on_export(self, sender, args):
        ...
        filepath = forms.save_file(file_ext=ext, files_filter=filter_str, default_name="PyLink Export")
        if not filepath:
            return
        ...
        for attempt in range(MAX_SAVE_RETRIES):
            try:
                ...
                if attempt < MAX_SAVE_RETRIES - 1:
                    ...
                    logger.error("PyLink export failed after {0} attempts: {1}".format(
                        MAX_SAVE_RETRIES, ex))
                    forms.alert(
        ...
        if not os.path.exists(filepath):
            forms.alert(
                ...
                "found at:\n{0}".format(filepath), title="PyLink")
    ...
    def on_import(self, sender, args):
        ...
        try:
            ...
            logger.error("PyLink import (read) failed: {0}".format(ex))
            forms.alert("Could not read file:\n{0}".format(ex), title="PyLink")
    ...
    def on_confirm(self, sender, args):
        ...
        t = Transaction(doc, "PyLink: Apply parameter changes")
        t.Start()
        applied = 0
        failed = []
        try:
            ...
            t.RollBack()
            logger.error("PyLink import failed: {0}".format(ex))
        ...
        if failed:
            for msg in failed:
                logger.warning("PyLink: skipped - {0}".format(msg))
