...
dispatching docx/odt reads to tools/format/ (the actual zip/XML
parsing lives there - see pylink_docx.py and pylink_odt.py), building
...
from pylink_docx import read_docx_sections
...
def read_word_sections(file_path):
    ...
    The actual docx/odt zip/XML parsing lives in tools/format/
    (pylink_docx.py / pylink_odt.py) - this just sniffs which format
    the file's zip actually is and hands off to the matching reader.
    """
    import zipfile
    try:
    ...
    if 'word/document.xml' in names:
        return read_docx_sections(file_path)
    elif 'content.xml' in names:
        return read_odt_sections(file_path)
    else:
        logger.debug(
