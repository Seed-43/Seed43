...
categories/schedules. Promoted out of PyLink since any tool that touches
...
def _try_register_enum(bip_name, enum_type_name):
    if BuiltInParameter is None or _DB is None:
        _log_info("PyLink dropdown table: BuiltInParameter/Autodesk.Revit.DB import failed entirely - "
                   "no enum-based dropdowns will work at all.")
        return
    try:
        bip = getattr(BuiltInParameter, bip_name)
    except AttributeError:
        _log_info("PyLink dropdown table: BuiltInParameter.{0} does not exist on this Revit "
                   "version - skipped.".format(bip_name))
        return
    try:
        enum_type = getattr(_DB, enum_type_name)
    except AttributeError:
        _log_info("PyLink dropdown table: Autodesk.Revit.DB.{0} does not exist on this Revit "
                   "version - skipped.".format(enum_type_name))
        return
    _ENUM_PARAM_TABLE[bip] = enum_type
    _log_info("PyLink dropdown table: registered {0} -> {1}".format(bip_name, enum_type_name))
...
def _get_built_in_parameter(param):
    try:
        ...
        _log_info("PyLink dropdown: '{0}'.Definition is not an InternalDefinition ({1}) - "
                   "can't check the enum table for it.".format(
                       getattr(definition, "Name", "?"), type(definition).__name__))
    except Exception as ex:
        _log_info("PyLink dropdown: _get_built_in_parameter failed: {0}".format(ex))
    return None
...
def _get_known_enum_options(param):
    ...
    if bip is None:
        _log_info("PyLink dropdown '{0}': not resolvable to a BuiltInParameter.".format(name))
        return None
    if bip not in _ENUM_PARAM_TABLE:
        _log_info("PyLink dropdown '{0}': BuiltInParameter is {1}, not in the registered "
    ...
    try:
        ...
        _log_info("PyLink dropdown '{0}': found in table ({1}), but Enum.GetValues "
                   "failed: {2}".format(name, enum_type, ex))
        return None
    if not options:
        _log_info("PyLink dropdown '{0}': enum table entry found but produced zero options.".format(name))
